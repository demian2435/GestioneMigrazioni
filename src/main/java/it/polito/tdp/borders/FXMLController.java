...
public class FXMLController {
	...
	void doCalcolaConfini(ActionEvent event) {
		...
		try {
			...
			boxNazione.getSelectionModel().selectFirst();
	...
	void doSimula(ActionEvent event) {
		...
		txtResult.appendText("Simulazione: " + partenza.getStateName() + "\n");
		txtResult.appendText("Numero passaggi: " + model.getT() + "\n");
		model.getStanziali().stream().filter(c -> c.getNumber() > 0)
				.forEach(c -> txtResult.appendText(c.getCountry() + " = " + c.getNumber() + "\n"));
	}
